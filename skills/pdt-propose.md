...
1. `[PM] 启动 REQ-4 计划编排`
2. 读取 `deliverables/{REQ-ID}/sa/design.md` 中的 Tasks 清单
3. 读取 `deliverables/{REQ-ID}/te/testcases.md` 中的用例列表
4. 编排执行计划，写入 `deliverables/{REQ-ID}/plan-action.md`
5. 更新 `deliverables/{REQ-ID}/.state.md`: current_step=REQ-4
...
3. 等待用户决策：
   - **通过**: 
     ...
     - 更新 `deliverables/{REQ-ID}/.state.md`: current_step=SR1-DONE, sr_status.SR1=approved
     - `[PM] SR1 通过，可执行 /pdt-apply`
   - **驳回**:
     - 记录驳回原因到 SR1-record.md
     - 根据驳回范围回退到对应步骤重新执行
     - 更新 `deliverables/{REQ-ID}/.state.md`: sr_status.SR1=rejected
