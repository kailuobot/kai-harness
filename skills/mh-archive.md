...
1. `[PM] 启动 ARC-5 更新交付记录`
2. 读取 `deliverables/{REQ-ID}/proposal.md` 第一行标题作为需求名称
3. 从 `.state.md` 中读取 output_type、mode、last_updated
4. 生成一行摘要（从 proposal.md 的"背景与目标"或"范围"中提炼，限 50 字内）
5. 追加一行到项目根目录 `DELIVERY-LOG.md` 的表格末尾：
   ```
   | {REQ-ID} | {需求名称} | {output_type} | {mode} | {完成日期} | {摘要} |
   ```
6. 若 `DELIVERY-LOG.md` 不存在，按以下格式创建后再追加：
   ```
   # 交付记录
