...
框架从 v0.4.0 起从"流程驱动"转向"质量驱动"。核心变化：每个角色不仅知道"做什么"，还知道"怎么做好"和"什么是不好的"。

### 17.2 三层质量注入

每个 Agent 定义（agents/*.md）包含三层质量指导：

| 层次 | 作用 | 触发时机 |
|------|------|---------|
| 思考框架 | 指导"先想什么再想什么" | 动笔前 |
| 反模式清单 | 明确"不要做什么" | 执行中自检 |
| 交付自检 | 提交前的内容质量 checklist | 填写回报前 |

这些指导是 Agent 定义的一部分，spawn SubAgent 时注入一次，不消耗额外流程步骤。

### 17.3 PM 质量门禁

PM 接收角色回报后，除文件存在性外，执行内容质量快扫（见 agents/pm.md）。质量门禁不做技术判断，只检查产出物的结构完整性和自洽性。

不满足时 PM 驳回并在新 handoff 中附带具体缺陷描述，避免低质量产出流入下游。

### 17.4 金标准示例

`templates/examples/` 目录存放每个角色的金标准产出示例。SubAgent spawn 时可将对应示例加入白名单，提供 few-shot 参考。
...
原修复循环是"TE 说失败 → DE 盲修 → 最多 5 轮"，缺乏方向性，容易发散。
...
PM 根因分析（提取失败特征 + 对比历史）
    ↓
PM 写入结构化修复上下文到 handoff
    ↓
DE 基于上下文定向修复
    ↓
TE 重新审计 → 更新 repair_history
    ↓
PM 判断收敛性 → 收敛则继续 / 发散则提前升级
```

### 18.3 收敛追踪

`.state.md` 中 `repair_history` 字段记录每轮修复的错误类型和失败数量。PM 据此判断：
- failed_count 递减 → 收敛，继续修复
- failed_count 递增连续 2 轮 → 发散，提前升级人工
- error_type 连续变化 → 修一个坏一个，提前升级

### 18.4 提前升级条件

不再机械等到第 5 轮，满足以下任一条件即升级：
...
原审批提示只给"文件路径 + 请确认"，人类缺乏判断依据，大多数时候只能盲批。
