...
项目采用"分布式设计文档"策略——设计意图分散在 agents（角色契约）、skills（操作规程）、templates（格式标准）中，本文档只是概述索引。

### 12.2 权威源映射

每个设计概念有且仅有一个权威源文件。完整映射见 `docs/source-of-truth.md`。

核心原则：
- CLAUDE.md 是最高约束
- skills/*.md 是执行权威
- agents/*.md 是角色契约
- scripts/*.sh 的退出码是最终判据
- 本文档（design.md）是设计参考，冲突时以上述文件为准

### 12.3 三层一致性保障

| 层次 | 机制 | 作用 |
|------|------|------|
| 预防 | check-harness.sh + handoff 协议 + 模板约束 | 结构化约束，防止格式偏离 |
| 发现 | verify.sh A/B/C/D + source-of-truth.md | 自动检测产出物问题 |
| 兜底 | SR1-4 审批 + PM 质量门禁 + 用户确认 | 人工评审最终把关 |

### 12.4 维护规则

- 修改 skills/agents 后，检查 design.md 对应章节是否需要同步
- 新增模板文件后，更新 source-of-truth.md 映射表
- 发现映射与实际不符时，以实际文件为准，更新映射表
